...
def scrape_codeup(check=True):
    ...
    if check == True:
        if os.path.isfile('data/codeup_blog_articles.csv'):
            return pd.read_csv('data/codeup_blog_articles.csv', index_col=[0])
        else:
            df = codeup_blog()
            df.to_csv('data/codeup_blog_articles.csv', index=False)
            return df
...
def scrape_news(check=True):
    ...
    if check == True:
        if os.path.isfile('data/indian_news_articles.csv'):
            return pd.read_csv('data/indian_news_articles.csv')
        else:
            df = indian_news()
            df.to_csv('data/indian_news_articles.csv', index = False)
            return df
